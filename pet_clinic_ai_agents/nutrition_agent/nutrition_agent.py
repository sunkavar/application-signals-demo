...
NUTRITION_SERVICE_URL = "http://aeb5c47baeace494bbf21a47d352baf7-1656987644.us-west-2.elb.amazonaws.com/api/nutrition/facts"
...
def get_nutrition_data(pet_type):
    ...
    try:
        ...
        # For non-200 status codes (including 500), raise an exception to record in span
        response.raise_for_status()
    except requests.HTTPError as e:
        # This will catch 4xx and 5xx errors including 500
        return {"facts": f"Error: Nutrition service returned {e.response.status_code}", "products": ""}
...
def create_nutrition_agent():
    ...
    return Agent(model=model, tools=tools, system_prompt=system_prompt, trace_attributes=trace_attributes)
